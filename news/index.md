...
- Optimized neighbourhood score calculation in `exir` by replacing
  ...
  calls with sparse adjacency matrix multiplication.
...
- Optimized the extraction of first- and second-order associated drivers
  for mediator tables by replacing repeated regex-based
  [`grep()`](https://rdrr.io/r/base/grep.html) searches with batched
  neighbourhood retrieval and set-based matching.
...
- Optimized `neighborhood.connectivity` by precomputing first-order
  ...
  [`igraph::neighborhood.size`](https://r.igraph.org/reference/neighborhood.size.html).
